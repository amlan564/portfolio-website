      ...
      >
        ...
        <div className="w-[60%] lg:w-[80%] xl:w-[60%] mx-auto grid grid-cols-1 lg:grid-cols-3 gap-20 items-center mt-20">
          {aboutInfo.map((item) => (
            <div
              key={item.id}
              className="w-full flex flex-col items-center text-center h-full transition-all duration-500 ease-in-out hover:scale-110"
            >
              <Image
                src={item.img}
                alt=""
                width={300}
                height={20}
                className="md:h-40 object-cover rounded-md mb-6"
              />
